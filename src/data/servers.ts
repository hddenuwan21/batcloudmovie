...
// 🤖 Unifed AI Chat Route (Handles both Chat Bot & Smart AI Synopsis Analysis)
app.post("/api/chat", async (req, res) => {
  ...
  // පෝලිමට ට්‍රයි කරන්න අවශ්‍ය මොඩල් ලිස්ට් එක
  const modelsToTry = [
    "gemini-2.0-flash",
    "gemini-2.5-flash",
    "gemini-flash-latest"
  ...
  // 1️⃣ ක්‍රමය 1: Official SDK (Fixed SDK Syntax)
  for (const modelName of modelsToTry) {
    try {
      ...
      // Smart AI Analysis එකෙන් Schema එකක් ආවොත් පමණක් JSON Output එක ඔන් කරයි
      if (responseSchema) {
        configObj.responseMimeType = "application/json";
        configObj.responseSchema = responseSchema;
      } else {
        configObj.systemInstruction = "You are a friendly cinema AI assistant on CineHub streaming platform. Reply concisely in friendly colloquial Sinhala or English depending on user message. Recommend movies to watch, ratings, and cast details.";
      }
  ...
  // 2️⃣ ක්‍රමය 2: OpenRouter Fallback Cascade
  for (const key of keysToTry) {
    try {
      ...
      if (orRes.ok) {
        ...
        if (reply) {
          console.log("Server: Success via OpenRouter Fallback.");
          return res.json({ reply });
        }
      }
    } catch (err) {
  ...
  // 3️⃣ ක්‍රමය 3: Direct API Fetch Fallback (No SDK dependency)
  ...
  // 4️⃣ ක්‍රමය 4: Ultimate Resilient Human Fallback
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // ඕනෑම URL එකකට 'dist/index.html' හෝ 'root/index.html' නිවැරදිව රවුට් කිරීම
    app.get("*", (req, res) => {
